...
<%
		...
		int number=1;
		String sql5="select id,user_id,product_id,qty from basket_view where user_id='"+pid+"'";
try {
...
%>
	...
	<form method="post" action="basket_modify.jsp?productnum=<%= id %>&<%=number%>"> 
		...
		<td  bgcolor="#e2edfa" align=right width=70><%= product_id %>원</td>
		<td bgcolor="#e2edfa" width=70 align=right><%= product_id*qty %> 원</td>
